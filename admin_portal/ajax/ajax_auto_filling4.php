...
// }
    ...
    // Function to check if there are any existing referrals
function checkReferralsExist($connection) {
...
//         // Check if there are any existing referrals
//         $referralsExist = checkReferralsExist($connection);

//         if (!$referralsExist) {
//             // If no referrals exist, insert the new user as the root
//             $sql = "INSERT INTO referrals (referrer_id, level) VALUES (:newUserId, 0)";
//             $stmt = $connection->prepare($sql);
//             $stmt->bindParam(':newUserId', $newUserId, PDO::PARAM_INT);
//             $stmt->execute();
//         } else {
//             // Find an available referrer based on level and max children
//             $referrerId = findAvailableReferrer($newUserId, $connection);

//             if ($referrerId !== null) {
//                 // Insert the new user under the available referrer
//                 // Insert the new user under the available referrer
//                 $insertionResult = insertUnderReferrer($newUserId, $referrerId, $connection);

//                 if ($insertionResult) {
//                     echo "User (ID: $newUserId) inserted under Referrer (ID: $referrerId).<br>";
//                 } else {
//                     echo "User (ID: $newUserId) could not be inserted.<br>";
//                 }
//             } else {
//                 echo "No available referrer for user (ID: $newUserId).<br>";
//             }
...
//             if ($referrerId !== null) {
//                 // Insert the new user under the available referrer
//                 // ... your existing code to insert the user ...
//                 $insertionResult = insertUnderReferrer($newUserId, $referrerId, $connection);
...
//                 }
//             } else {
//                 // If no available referrer in the current level, create a new level
//                 $nextLevel = findNextAvailableLevel($connection);
//                 $referrerId = findAvailableReferrerAtLevel($newUserId, $nextLevel, $connection);

//                 if ($referrerId !== null) {
//                     // Insert the new user under the available referrer at the new level
//                     // ... your existing code to insert the user ...
//                     $insertionResult = insertUnderReferrer($newUserId, $referrerId, $connection);
...
// function insertRootReferrer($newUserId, $connection) {
//     $sql = "INSERT INTO referrals (referrer_id, level) VALUES (:newUserId, 0)";
//     $stmt = $connection->prepare($sql);
//     $stmt->bindParam(':newUserId', $newUserId, PDO::PARAM_INT);
//     $stmt->execute();
...
//         if (!$referralsExist) {
//             // If no referrals exist, insert the new user as the root
//             insertRootReferrer($newUserId, $connection);
//         } else {
//             // Find an available referrer based on level and max children
//             $referrerId = findAvailableReferrer($newUserId, $connection);
...
//         if (!$referralsExist) {
//             // No referrals exist, insert the new user as the root (level 0)
//             insertRootReferrer($newUserId, $connection);
//         } else {
//             // Find an available referrer based on level and max children
//             $referrerId = findAvailableReferrer($newUserId, $connection);
...
//                 }
//             } else {
//                 echo "No available referrer for user (ID: $newUserId). Moving to the next level.<br>";
//             }
//         }
...
// Loop through the new user IDs and insert them in the hierarchy
foreach ($newUserIds as $newUserId) {
    insertUserHierarchy($newUserId, $connection);
}
